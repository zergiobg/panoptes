...
import { Map, LogIn, Briefcase, Plus, Shield, Bell, MapPin, Clock } from 'lucide-react';
    ...
    <main style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <nav style={{
        ...
        <div className="mobile-nav-gap" style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
          <Link href={`/${locale}/notificaciones`} style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '6px', whiteSpace: 'nowrap' }}>
            <Bell size={16} /> <span className="hide-on-mobile">{t('nav.notifications')}</span>
          </Link>
          <Link href={`/${locale}/mi-actividad`} style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '6px', whiteSpace: 'nowrap' }}>
            <Briefcase size={16} /> <span className="hide-on-mobile">{t('nav.activity')}</span>
          </Link>
          <Link href={`/${locale}/map`} style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', transition: 'color 0.2s', display: 'flex', alignItems: 'center', gap: '6px', whiteSpace: 'nowrap' }}>
            <Map size={16} /> <span className="hide-on-mobile">{t('nav.map')}</span>
          </Link>
          
          {isAdmin && (
            <>
              <Link href={`/${locale}/admin`} style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '6px', whiteSpace: 'nowrap', position: 'relative' }}>
                <Shield size={16} /> 
      ...
      <section className="mobile-padding hero-padding" style={{
        ...
        <div className="animate-in" style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
          ...
          <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '30px' }}>
            <Link href={`/${locale}/reportar`} className="btn-primary" style={{ padding: '16px 40px', fontSize: '1.1rem', boxShadow: '0 8px 24px rgba(255, 126, 51, 0.4)', display: 'flex', alignItems: 'center', gap: '10px', textDecoration: 'none' }}>
              <Plus size={20} /> {t('hero.cta')}
            </Link>
          </div>
      ...
      <section style={{ padding: '0 20px', maxWidth: '1200px', margin: '0 auto', width: '100%', flex: 1 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '30px' }}>
          {[
            { key: 'all', dbValue: 'Todos' },
            { key: 'person', dbValue: 'Persona' },
            { key: 'pet', dbValue: 'Mascota' },
            { key: 'vehicle', dbValue: 'Vehículo' },
            { key: 'keys', dbValue: 'Llaves' },
            { key: 'devices', dbValue: 'Dispositivos' },
            { key: 'personal', dbValue: 'Artículo personal' },
            { key: 'clothing', dbValue: 'Ropa' },
            { key: 'document', dbValue: 'Documento' },
            { key: 'other', dbValue: 'Otro' }
          ].map(({ key, dbValue }) => (
            <button
              key={dbValue}
              ...
              style={{
                ...
                background: categoryFilter === dbValue ? 'var(--accent-main)' : 'rgba(0,0,0,0.3)',
                color: categoryFilter === dbValue ? '#000' : 'var(--text-secondary)'
              }}
            >
              {t(`filters.${key}`)}
            </button>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '60px', color: 'var(--text-secondary)' }}>
            <div className="pulse" style={{ fontSize: '1.5rem', marginBottom: '16px' }}>{t('feed.loading')}</div>
          </div>
        ) : reports.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px', color: 'var(--text-secondary)' }}>
            <p>{t('feed.empty')}</p>
